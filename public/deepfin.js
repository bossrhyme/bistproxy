...
// ═══════════════════════════════════════════
// STRATEJİ GETİRİ PANELİ
// Yüklü tarama verisi üzerinde her stratejinin/filtrenin bugünkü
// filtre kriterlerinden geçen hisselerin son 1/3/6 ay ortalama
// getirisini hesaplar. Eşit ağırlıklı, geçmişe dönüktür; "bu
// filtreyi geçen hisseler son dönemde ne yaptı" sorusunu yanıtlar.
// ═══════════════════════════════════════════
var _perfRuleByKey = null;

function _perfBuildRuleMap() {
  if (_perfRuleByKey) return _perfRuleByKey;
  _perfRuleByKey = {};
  FILTER_RULES.forEach(function(r) {
    if (r[1]) _perfRuleByKey[r[1]] = { field: r[0], isMin: true };
    if (r[2]) _perfRuleByKey[r[2]] = { field: r[0], isMin: false };
  });
  return _perfRuleByKey;
}

// applyAndRender ile aynı eşleşme semantiği: teknik alan null ise filtre
// atlanır, temel alan null ise hisse elenir.
function _perfMatch(s, filters, special) {
  var map = _perfBuildRuleMap();
  for (var key in filters) {
    ...
    if (raw === null || raw === undefined) {
      if (TECH_NULL_SKIP_FIELDS.indexOf(rule.field) !== -1) continue;
      return false;
    }
...
function _perfStats(filters, special) {
  var n = 0, s1 = 0, c1 = 0, s3 = 0, c3 = 0, s6 = 0, c6 = 0;
  for (var i = 0; i < allData.length; i++) {
    var s = allData[i];
    if (!_perfMatch(s, filters, special)) continue;
    n++;
    if (s.perf1m !== null && s.perf1m !== undefined) { s1 += s.perf1m; c1++; }
    if (s.perf3m !== null && s.perf3m !== undefined) { s3 += s.perf3m; c3++; }
    if (s.perf6m !== null && s.perf6m !== undefined) { s6 += s.perf6m; c6++; }
  }
  return { n: n, p1: c1 ? s1 / c1 : null, p3: c3 ? s3 / c3 : null, p6: c6 ? s6 / c6 : null };
}

// Yatırımcı profili = preset+tech+goat filtre birleşimi (applyProfile ile aynı)
function _perfProfileFilters(profile) {
  var merged = {}, special = null;
  profile.goat.forEach(function(k) { var g = GURUS[k]; if (g) { Object.assign(merged, g.filters); if (g.special) special = g.special; } });
  profile.preset.forEach(function(k) { var p = PRESETS[k]; if (p) Object.assign(merged, p.filters); });
  profile.tech.forEach(function(k) { var p = TECH_PRESETS[k]; if (p) Object.assign(merged, p.filters); });
  return { filters: merged, special: special };
}

function _perfFmt(v) {
  if (v === null || v === undefined) return '<span class="perf-na">—</span>';
  var cls = v >= 0 ? 'perf-pos' : 'perf-neg';
  return '<span class="' + cls + '">' + (v >= 0 ? '+' : '') + v.toFixed(1) + '%</span>';
}

function _perfRowHtml(icon, label, st, sub) {
  return '<div class="perf-row' + (st.n === 0 ? ' perf-row-empty' : '') + '">' +
    '<span class="perf-row-icon">' + icon + '</span>' +
    '<span class="perf-row-name">' + label + (sub ? '<span class="perf-row-sub">' + sub + '</span>' : '') + '</span>' +
    '<span class="perf-row-n">' + st.n + ' hisse</span>' +
    '<span class="perf-row-val">' + _perfFmt(st.p1) + '</span>' +
    '<span class="perf-row-val">' + _perfFmt(st.p3) + '</span>' +
    '<span class="perf-row-val">' + _perfFmt(st.p6) + '</span>' +
  '</div>';
}

function _perfSection(title, items) {
  // items: [{icon,label,st,sub}] — 3 aylık getiriye göre sırala, boşlar sona
  ...
  return '<div class="perf-section-hd">' + title + '</div>' +
    '<div class="perf-head-row"><span></span><span></span><span>Adet</span><span>1A</span><span>3A</span><span>6A</span></div>' +
    items.map(function(it) { return _perfRowHtml(it.icon, it.label, it.st, it.sub); }).join('');
}

var _perfShowAll = false;

function _perfRenderBody() {
  var body = document.getElementById('perf-modal-body');
  if (!body) return;
  if (!allData.length) {
    body.innerHTML = '<div class="perf-empty-msg">Veri yok. Önce bir tarama çalıştırın.<br><br>' +
      '<button class="perf-retry-btn" onclick="openPerfPanel()">↻ Tekrar Dene</button></div>';
    return;
  }

  var exMeta = EXCHANGE_META[currentExchange] || EXCHANGE_META.bist;
  var html = '<div class="perf-note">' + exMeta.flag + ' <b>' + exMeta.name + '</b> · ' + allData.length + ' hisse tarandı. ' +
    'Her stratejinin <b>bugünkü kriterlerinden geçen</b> hisselerin son 1 / 3 / 6 aydaki eşit ağırlıklı ortalama getirisi. ' +
    'Geçmişe dönük bir göstergedir, gelecek getiriyi garanti etmez.</div>';

  // Varsayılan: 9 yatırımcı stratejisi
  var profItems = INVESTOR_PROFILES.map(function(p) {
    var pf = _perfProfileFilters(p);
    return { icon: p.icon, label: p.label, st: _perfStats(pf.filters, pf.special), sub: '' };
  });
  html += _perfSection('🧬 Yatırımcı Stratejileri', profItems);

  if (_perfShowAll) {
    var goatItems = Object.keys(GURUS).map(function(k) {
      var g = GURUS[k];
      return { icon: '🐐', label: g.label, st: _perfStats(g.filters, g.special), sub: '' };
    });
    html += _perfSection('🏆 GOAT Stratejileri', goatItems);

    var presetItems = Object.keys(PRESETS).map(function(k) {
      var p = PRESETS[k];
      return { icon: '📊', label: p.label, st: _perfStats(p.filters, null), sub: '' };
    });
    html += _perfSection('📐 Temel Filtreler', presetItems);

    var techItems = Object.keys(TECH_PRESETS).map(function(k) {
      var p = TECH_PRESETS[k];
      return { icon: '📈', label: p.label, st: _perfStats(p.filters, null), sub: '' };
    });
    html += _perfSection('⚙️ Teknik Filtreler', techItems);

    html += '<button class="perf-toggle-all" onclick="_perfToggleAll()">▲ Sadece Yatırımcı Stratejileri</button>';
  } else {
    var totalExtra = Object.keys(GURUS).length + Object.keys(PRESETS).length + Object.keys(TECH_PRESETS).length;
    html += '<button class="perf-toggle-all" onclick="_perfToggleAll()">▼ Tüm Filtre &amp; Stratejileri Göster (' + totalExtra + ')</button>';
  }

  body.innerHTML = html;
}

function _perfToggleAll() {
  _perfShowAll = !_perfShowAll;
  _perfRenderBody();
}

function _perfEnsureModal() {
  if (document.getElementById('perf-modal')) return;
  var div = document.createElement('div');
  div.className = 'modal-overlay';
  div.id = 'perf-modal';
  div.onclick = function(e) { if (e.target === div) closePerfPanel(); };
  div.innerHTML =
    '<div class="modal perf-modal-box" onclick="event.stopPropagation()">' +
      '<div class="modal-head">' +
        '<div class="modal-title">STRATEJİ <span>GETİRİLERİ</span></div>' +
        '<button class="modal-close" onclick="closePerfPanel()">✕</button>' +
      '</div>' +
      '<div class="modal-body" id="perf-modal-body"></div>' +
    '</div>';
  document.body.appendChild(div);
}
...
function openPerfPanel() {
  ...
  if (allData.length) { _perfRenderBody(); return; }

  // Veri yok — taramayı tetikle ve tamamlanınca hesapla
  body.innerHTML = '<div class="perf-empty-msg"><div class="pf-spin-mini"></div>Piyasa verisi yükleniyor…</div>';
  var renderWhenReady = function() {
    var tries = 0;
    var iv = setInterval(function() {
      tries++;
      if (allData.length) { clearInterval(iv); _perfRenderBody(); }
      else if (tries > 60 || !_scanRunning) {
        if (!allData.length && !_scanRunning) { clearInterval(iv); _perfRenderBody(); }
        if (tries > 60) { clearInterval(iv); _perfRenderBody(); }
      }
    }, 700);
  };
  if (_scanRunning) { renderWhenReady(); }
  else { try { runScan().then(renderWhenReady).catch(renderWhenReady); } catch (e) { renderWhenReady(); } }
}
